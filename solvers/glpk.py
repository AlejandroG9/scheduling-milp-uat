...
REPORTE_PATH = Path(__file__).resolve().parents[1] / "results" / "reporte_glpk.csv"
REPORTE_PATH.parent.mkdir(parents=True, exist_ok=True)


def resolver_modelo_glpk(modelo, datos, permutacion):
    """
    Resuelve un ConcreteModel de Pyomo con GLPK,
    ...
    with etapa_registrada("Optimización GLPK", iteracion_actual, modelo=modelo, version="constructor_decorador",
                          registros=registros, semestre=permutacion[-1], permutacion=permutacion):
        opt = SolverFactory('glpk')
        opt.options['tmlim'] = 3600  # límite de tiempo en segundos
    ...
    if tc == tc_enum.infeasible:
        ...
        with open(diag_path / f"diagnostico_glpk_{permutacion[-1]}.txt", "w") as f:
            logging_logger = logging.getLogger(f"diagnostico_glpk_{permutacion[-1]}")
    ...
    sol_csv = sol_dir / f"Solucion_GLPK_{rama}.csv"
    pd.DataFrame.from_dict(sol_dict, orient="index", columns=["Valor"]).to_csv(sol_csv)
    print(f"✅ Solución GLPK guardada en {sol_csv}")
