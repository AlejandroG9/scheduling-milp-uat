...
REPORTE_PATH = Path(__file__).resolve().parents[1] / "results" / "reporte_highs.csv"
REPORTE_PATH.parent.mkdir(parents=True, exist_ok=True)


def resolver_modelo_highs(modelo, datos, permutacion):
    """
    Resuelve un ConcreteModel de Pyomo con HiGHS,
    guarda métricas y solución en CSVs.

    HiGHS es un solver MILP moderno y gratuito, competitivo con solvers comerciales.
    ...
    with etapa_registrada("Optimización HiGHS", iteracion_actual, modelo=modelo, version="constructor_decorador",
                          registros=registros, semestre=permutacion[-1], permutacion=permutacion):
        opt = SolverFactory('appsi_highs')
        opt.options['time_limit'] = 3600.0
    ...
    if tc == tc_enum.infeasible:
        ...
        with open(diag_path / f"diagnostico_highs_{permutacion[-1]}.txt", "w") as f:
            logging_logger = logging.getLogger(f"diagnostico_highs_{permutacion[-1]}")
    ...
    sol_csv = sol_dir / f"Solucion_HiGHS_{rama}.csv"
    pd.DataFrame.from_dict(sol_dict, orient="index", columns=["Valor"]).to_csv(sol_csv)
    print(f"✅ Solución HiGHS guardada en {sol_csv}")
